...
func create_audio(type: SoundEffectSettings.SOUND_EFFECT_TYPE):
	if sound_effect_dict.has(type):
		...
		if sound_effect_setting.has_open_limit():
			...
			new_audio.play()
			print(new_audio.playing)

func create_audio_loser():
	randomize()
	var r = randi()
	if r % 2 == 0:
		create_audio(SoundEffectSettings.SOUND_EFFECT_TYPE.DO_IT_AGAIN)
	else:
		create_audio(SoundEffectSettings.SOUND_EFFECT_TYPE.TOO_EASY)
